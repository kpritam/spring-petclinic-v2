...
type AssistantMessage = {
  ...
  /** Seconds spent inside a `<think>` block, if the model emitted one. See `./thinking.ts`. */
...
const MessageBubble = memo(function MessageBubble(props: {
  ...
  const parsed = msg.error ? null : splitThinking(msg.text);
  return (
    <div className={styles.bubbleRow}>
      ...
      >
        {msg.error ? (
          ...
          <>
            {parsed && parsed.reasoning !== null ? (
              <ThinkingBlock
                text={parsed.reasoning}
                isThinking={false}
                durationSeconds={msg.thinkingSeconds ?? null}
              />
            ) : null}
            {parsed?.answer ? <Markdown source={parsed.answer} /> : null}
          </>
        ) : (
...
export default function SpellbookChat(): ReactNode {
  ...
  /** Wall-clock start of the current `<think>` block; `null` when not (yet) thinking. */
  const thinkingStartRef = useRef<number | null>(null);
  /** Frozen elapsed seconds once `</think>` closes (or the stream ends mid-block). */
  const thinkingSecondsRef = useRef<number | null>(null);
  /** How much of the answer (post-`</think>`) has already been handed to TTS — see `onToken`. */
  const ttsAnswerConsumedRef = useRef(0);
  /**
   * Identifies the "current" turn. `streamingId`/`streamingText`/`busy` are
   * singleton state shared across turns, so a cancelled turn's `ask()` —
   * which resolves normally instead of rejecting, see below — must not be
   * allowed to write into them once a newer turn has started. Bumped once
   * per `submitTurn` call; any async continuation checks it's still the
   * latest generation before touching shared state.
   */
  ...
  const submitTurn = useCallback(
    async (rawText: string, turnOpts: { readonly speak: boolean }) => {
      ...
      // This turn's identity. `engine.ask()` resolves normally even when
      // cancelled (finishReason: "abort") — it never rejects — so a stale
      // turn left running in the background after Clear/Close/barge-in
      // must check `isCurrent()` before it's allowed to touch any of the
      // shared streaming/messages/busy state below.
      const myGen = ++turnGenRef.current;
      const isCurrent = (): boolean => turnGenRef.current === myGen;
      ...
      thinkingSecondsRef.current = null;
      ttsAnswerConsumedRef.current = 0;
      setBusy(true);
      ...
      // are still useful as signal to the model so we keep them. Assistant
      // turns are stripped of any `<think>` block — see `stripThinking`.
      const history = messages
        ...
        .map((m) => ({
          role: m.role,
          content: m.role === "assistant" ? stripThinking(m.text) : m.text,
        }));
      ...
      try {
        const result = await engine.ask(text, {
          ...
          onToken: (ev) => {
            ...
            // Track the `<think>` block's wall-clock duration and make sure
            // TTS only ever hears the answer, never the reasoning trace.
            const parsed = splitThinking(streamingTextRef.current);
            if (parsed.isThinking && thinkingStartRef.current === null) {
              ...
              thinkingSecondsRef.current = Math.max(
                1,
                Math.round((Date.now() - thinkingStartRef.current) / 1000),
              );
            }
            if (turnOpts.speak) {
              const newAnswerPiece = parsed.answer.slice(
                ttsAnswerConsumedRef.current,
              );
              ttsAnswerConsumedRef.current = parsed.answer.length;
              if (newAnswerPiece) {
                ttsBufferRef.current = flushSentences(
                  ttsBufferRef.current + newAnswerPiece,
                  speakChunk,
                );
              }
            }
        ...
        if (!isCurrent()) {
          // Superseded by a newer turn (Clear/Close/barge-in started one
          // while this request was still in flight) — drop the result
          // instead of resurrecting it into the active conversation.
          return;
        }
        ...
        if (result?.finishReason === "abort") {
          // Discard any partial/unclosed reasoning trace rather than
          // rendering it verbatim in the red error bubble — it's
          // necessarily incomplete, and `MessageBubble` never runs
          // `ThinkingBlock` for `error` messages.
          const stoppedAnswer = stripThinking(finalText);
          const body = stoppedAnswer
            ? `${stoppedAnswer}\n\n(stopped)`
            : "(stopped)";
          setMessages((m) => [
            ...m,
            { id: assistantId, role: "assistant", text: body, error: true },
          ]);
        } else {
          ...
          setMessages((m) => [
            ...m,
            {
              ...
              truncated,
              thinkingSeconds: thinkingSecondsRef.current ?? undefined,
            },
          ]);
        }
        setStreamingId(null);
  ...
  const parsedStream = splitThinking(streamingText);
                  ...
                  {streamingId ? (
                    <div key={streamingId} className={styles.bubbleRow}>
                      ...
                      >
                        {streamingText ? (
                          <>
                            {parsedStream.reasoning !== null ? (
                              <ThinkingBlock
                                text={parsedStream.reasoning}
                                isThinking={parsedStream.isThinking}
                                durationSeconds={thinkingSecondsRef.current}
                              />
                            ) : null}
                            {parsedStream.answer ? (
                              <Markdown source={parsedStream.answer} streaming />
                            ) : null}
                          </>
                        ) : (
                          <span className={styles.typingDots} aria-hidden>
