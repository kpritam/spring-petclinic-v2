  ...
  return {
    ...
    async *stream(
      ...
      const result = streamText({
        ...
        // The AI SDK processes some stream failures internally rather than
        // throwing them from `textStream`/`finishReason`/`usage`. Without
        // this callback wired up, those failures can surface as unhandled
        // promise rejections instead of reaching the try/catch below.
        onError: ({ error }) => {
          if (typeof console !== "undefined") {
            console.error("[spellbook-chat] stream error", error);
          }
        },
      });
